...
describe('check auth reducer', () => {
  ...
  test('should change on action call of REGISTER_SUCCESS', () => {
    const expectedPayload = { isRegisterSuccess: true, isRequestingRegister: false };
...
/*
  test('should change on action call of REGISTER_FAILURE', () => {
    ...
    const expectedPayload = { isRegisterSuccess: false, isRequestingRegister: false, err: errorMessage };
    expect(authReducer({}, { type: REGISTER_FAILURE })).toEqual(
      expectedPayload
    );
  });*/
  ...
  test('should change on action call of SET_LOGGEDOUT', () => {
    ...
    expect(authReducer({}, { type: SET_LOGGEDOUT })).toEqual(
      expectedPayload
    );
  });
...
/*
  test('should change on action call of SHOW_APIMESSAGE', () => {
    // может быть тут не toequal, а по другому? сообщение то приходит 
    const expectedPayload = { apiData: 'test api message' };
    expect(authReducer({}, { type: SHOW_APIMESSAGE })).toEqual(
      expectedPayload
    );
  });
*/
